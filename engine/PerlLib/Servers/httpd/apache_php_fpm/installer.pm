...
sub _buildMasterVhostFiles
{
	...
	$rs = $self->{'httpd'}->apacheBkpConfFile("$self->{'config'}->{'HTTPD_SITES_AVAILABLE_DIR'}/00_master.conf");
	return $rs if $rs;
	...
	if($main::imscpConfig{'PANEL_SSL_ENABLED'} eq 'yes') {
		...
		$rs = $self->{'httpd'}->apacheBkpConfFile("$self->{'config'}->{'HTTPD_SITES_AVAILABLE_DIR'}/00_master_ssl.conf");
		return $rs if $rs;
	...
	for('default', 'default-ssl', '000-default.conf', 'default-ssl.conf') {
		$rs = $self->{'httpd'}->disableSite($_) if -f "$self->{'config'}->{'HTTPD_SITES_AVAILABLE_DIR'}/$_";
		return $rs if $rs;
	}
