...
function add_user($tpl)
{
	...
	if (is_xhr()) { // Password generation (AJAX request)
		...
		if (check_user_data()) {
			...
			$tpl->assign(
				array(
					'EMAIL' => tohtml($_POST['email']),
					'USERNAME' => tohtml($_POST['username']),
					'FIRST_NAME' => tohtml($_POST['fname']),
					'LAST_NAME' => tohtml($_POST['lname']),
					'FIRM' => tohtml($_POST['firm']),
					'ZIP' => tohtml($_POST['zip']),
					'CITY' => tohtml($_POST['city']),
					'STATE' => tohtml($_POST['state']),
					'COUNTRY' => tohtml($_POST['country']),
					'STREET_1' => tohtml($_POST['street1']),
					'STREET_2' => tohtml($_POST['street2']),
					'PHONE' => tohtml($_POST['phone']),
					'FAX' => tohtml($_POST['fax']),
