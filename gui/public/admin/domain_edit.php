 ...
 */
function admin_getResellerProps($resellerId)
{
	$query = '
		SELECT
 ...
 */
function admin_getDomainProps($domainId)
{
	$query = "
		SELECT
...
function _admin_generateLimitsForm($tpl, &$data)
{
	...
	// Disk space limit
	$tplVars['TR_DISK_LIMIT'] = tr('Disk space limit [MiB]') . '<br/><i>(0 ' . tr('unlimited') . ')</i>';
